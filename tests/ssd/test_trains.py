...
def train(resolution: tuple[int, int], tmp_path: pathlib.Path) -> list[Sample]:
    # A single image with a black dot in the center, sized to match
    # build_ssdlite's smallest anchor (32px, see ssd/lite.py), repeated
    # across several samples so there's more than one batch per epoch.
    h, w = resolution
